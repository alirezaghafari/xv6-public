...
fork(void)
{
  ...
  // there is only 1 thread for the child because in fork, fork creates a new pgdir for the thread
  np->threads = 1;  
  np->parent = curproc;
  *np->tf = *curproc->tf;
...
int clone(void *stack){
  ...
  struct proc *curproc = myproc();
  // np is the new process , basically a process is out thread
...
int join(void){
  cprintf("Start running Thread wait ... \n");
  struct proc *p;
  int havekids, pid;
  struct proc *curproc = myproc();

  void **stk;
  if(argptr(0, (void*)&stk, sizeof(stk) == -1)) {
    cprintf("Stack empty\n");
		return -1;
  }
